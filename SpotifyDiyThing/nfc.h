...
bool nfcSetup(SpotifyArduino *spotifyObj, SpotifyDisplay *theDisplay)
{

  spotify_nfc = spotifyObj;
  nfc_Display = theDisplay;
...
bool updateSpotify(char *tagContent)
{
  ...
  if (strncmp(tagContent + 1, "open.spotify.com", 16) == 0)
  { // The +1 is cause first charcter indicated protocol (I think), not needed anyways
    return handleSpotifyUrl(tagContent + 1);
  }
  else if (strncmp(tagContent + 8, "open.spotify.com", 16) == 0)
  { // In case it's written as plain text, skipping the "https://"
...
bool handleTag(const char *trackUri)
{
    ...
    if (recordCount > 0)
    {
      for (int i = 0; i < recordCount; i++)
      {
        ...
        if (payloadLength > 0)
        {
          ...
          nfc_Display->markDisplayAsTagRead();
  ...
  if (writeTag && nfc.tagPresent())
  {
    ...
    if (success)
    {
      nfc_Display->markDisplayAsTagWritten();
      Serial.println("Success. Try reading this tag with your phone.");
      forceUpdate = true; // To remove the squares
    }
    else
    {
